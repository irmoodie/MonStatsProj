...
summary(lm9) 
anova(lm8,lm9) # min. ad. mod.

lm10<-update(lm9, .~. -light:species:time_after_germ)
anova(lm9,lm10) # supported to be kept in
...
re2 <- lmer(Rosette_size ~ light + species + Cotyledons + time_after_germ + 
    light:species + light:time_after_germ + light:species:time_after_germ+(0+species|population), 
    data = rosette_data)

re3 <- lmer(Rosette_size ~ light + species + Cotyledons + time_after_germ + 
    light:species + light:time_after_germ + light:species:time_after_germ+(1|population), 
    data = rosette_data)

AICc(lm9, re3, re2, re1)

rm(lm1,lm2,lm3,lm4,lm5,lm6,lm7,lm8, lm10, re1, re2, re3)
